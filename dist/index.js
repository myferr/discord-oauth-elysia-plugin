...
export function discordOAuth(config) {
    ...
    return new Elysia()
        ...
        if (!code) {
            return new Response(JSON.stringify({ error: 'Missing "code" query parameter' }), { status: 400, headers: { "Content-Type": "application/json" } });
        }
        if (!clientId || !clientSecret || !redirectUri) {
            return new Response(JSON.stringify({ error: "Discord OAuth is not configured" }), { status: 500, headers: { "Content-Type": "application/json" } });
        }
        try {
